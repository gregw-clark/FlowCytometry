...
#	sys.exit()
	...
	for mouse in doOrder:
		...
		scoreZ=QCmouse(doOrder[mouse],theIMPC)
		scoreZ=[min(5,l) for l in scoreZ]
		#print mouse,scoreZ
		Morder.append(mouse)
		Mdata.append(scoreZ)
#		if len(outs):
...
#		print PfileA,PfileB
	...
	k = {'family':"Courier New, monospace",'size' : 2,'color' : "#7f7f7f"}
	xax = {'title':'Parameters','titlefont':k}
	yax = {'title':'Mice','titlefont':k}
	...
	py.image.save_as(fig, filename='DataMouse.png')
	
	prp=max(sizesM.values())
#	print "Data entries of:", list(set(sizesM.values()))
	for m in doOrder:
		if len(doOrder[m]) != prp:
			print "\t\t***Wrong size for Mouse %s. Should be %s and we find %s.***" % (m,prp,len(doOrder[m]))
#	
	f.close()
