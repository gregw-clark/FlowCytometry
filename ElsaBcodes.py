...
if __name__ == "__main__":
	...
	connA,impcdb=dbins.connect(db_env='PROD')
...
#		print PfileA,PfileB
		...
		if wStrain == "B6NC":
			...
			impcdb.execute("""select COLONYNAME from impc.impc_Mouseline where straincode = %s""" ,(wStrain,))
			colony=impcdb.fetchall()[0]

		try:			
			impcdb.execute("""select ml.GENESYMBOL,e.zygosity,e.Gender,e.IMPC_specimen_Id,e.MouseBCode,s.SpleenWeight from impc.impc_Mice e join impc.impc_Mouseline ml
			 on e.mouseline = ml.mouseline_id
			 left join musmusculims.impc_spleenweight s on
			 e.MouseBCode = s.MouseBCode 
		...
		if not retimpc:
			print wStrain,wTag,"MISSING"
			geneSymbol,zygosity,gender,dcc_id,MouseBcode,spleenweight='','','','','',''
		else:
			geneSymbol,zygosity,gender,dcc_id,MouseBcode,spleenweight=retimpc
		...
		if colony[0] is None:
			colony=[""]	
		try:
			print wDate,wStrain,wTag,MouseBcode,spleenweight,type(spleenweight)
			#stringdata=",".join([wDate,wStrain,MouseBcode,spleenweight,colony[0],wTag,genotype,gender,dcc_id,PfileA,PfileB])
			stringdata=",".join([wDate,wStrain,wTag,MouseBcode,str(spleenweight)])
		except:
			print "MISSING",wDate,wStrain,colony[0],wTag,genotype,gender,dcc_id,PfileA,PfileB
